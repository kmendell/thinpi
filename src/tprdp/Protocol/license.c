...
licence_generate_hwid(uint8 *hwid)
{
	...
	strncpy((char *)(hwid + 4), tpHostname, LICENCE_HWID_SIZE - 4);
}
			 ...
			 uint8 *licence_data, int licence_size, uint8 *hwid, uint8 *signature)
{
	...
	out_uint8(s, ((tpRDPVersion >= RDP_V5) ? 3 : 2)); /* version */
...
licence_process_request(STREAM s)
{
	...
	licence_send_new_licence_request(null_data, null_data, tpUsername, tpHostname);
}
