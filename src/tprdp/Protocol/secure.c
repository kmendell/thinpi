...
extern char tpHostname[16];
extern uint32 tpSessionWidth;
extern uint32 tpSessionHeight;
extern int tpDPI;
extern unsigned int tpKBLayout;
extern int tpKBType;
extern int tpKBSubType;
extern int tpKBFuncKeys;
...
extern RD_BOOL tpConsoleSession;
...
{
	int length = 162 + 76 + 12 + 4 + (tpDPI > 0 ? 18 : 0);
	unsigned int i;
	...
	out_uint16_le(s, 216 + (tpDPI > 0 ? 18 : 0)); /* length */
	out_uint32_le(s, rdpversion);				  /* version */
	out_uint16_le(s, tpSessionWidth);  /* desktopWidth */
	out_uint16_le(s, tpSessionHeight); /* desktopHeight */
	out_uint16_le(s, RNS_UD_COLOR_8BPP);		  /* colorDepth */
	out_uint16_le(s, RNS_UD_SAS_DEL);			  /* SASSequence */
	out_uint32_le(s, tpKBLayout);				  /* keyboardLayout */ 
	/*
	...
	out_utf16s_padded(s, tpHostname, 32, 0x00);

	out_uint32_le(s, tpKBType);		   /* keyboardType */
	out_uint32_le(s, tpKBSubType);	   /* keyboardSubtype */
	out_uint32_le(s, tpKBFuncKeys); /* keyboardFunctionKey */
	...
	out_uint16_le(s, MIN(tpServerDepth, 24));
	if (tpServerDepth == 32)
	...
	if (tpDPI > 0)
	{
		...
		utils_calculate_dpi_scale_factors(tpSessionWidth,
										  tpSessionHeight, tpDPI, &physwidth,
...
sec_process_crypt_info(STREAM s)
{
	...
	generate_random(tpClientRandom);
	sec_rsa_encrypt(g_sec_crypted_random, tpClientRandom, SEC_RANDOM_SIZE,
					g_server_public_key_len, modulus, exponent);
	sec_generate_keys(tpClientRandom, server_random, rc4_key_size);
}
...
sec_recv(RD_BOOL *is_fastpath)
{
	...
	while ((s = mcs_recv(&channel, is_fastpath, &fastpath_hdr)) != NULL)
	{
		...
		if (tpEncryption || (!g_licence_issued && !g_licence_error_result))
		{
			data_offset = s_tell(s);
