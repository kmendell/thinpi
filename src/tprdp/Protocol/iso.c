...
extern RD_BOOL tpEncryptionInitial;
extern RDP_VERSION tpRDPVersion;
extern RD_BOOL tpPasswordASPin;

static RD_BOOL g_negotiate_rdp_protocol = True;

extern char *tpSCCSPName;
extern char *tpSCReaderName;
extern char *tpSCCardName;
extern char *tpSCContainerName;
extern char tpTLSVersion[];
...
iso_send_connection_request(char *username, uint32 neg_proto)
{
	...
	if (tpRDPVersion >= RDP_V5 && g_negotiate_rdp_protocol)
		length += 8;
...
retry:
	*selected_protocol = PROTOCOL_RDP;
	...
	if (tpRDPVersion >= RDP_V5 && s_check_rem(s, 8))
	{
...
{
	tpEncryption = tpEncryptionInitial;
	tcp_reset_state();
}
