...
tcp_tls_connect(void)
{
	...
	else if (!strcmp(tpTLSVersion, "1.0"))
		priority = GNUTLS_PRIORITY ":-VERS-ALL:+VERS-TLS1.0";
	else if (!strcmp(tpTLSVersion, "1.1"))
		priority = GNUTLS_PRIORITY ":-VERS-ALL:+VERS-TLS1.1";
	else if (!strcmp(tpTLSVersion, "1.2"))
...
tcp_connect(char *server)
{
	...
	{
		snprintf(tcp_port_rdp_s, 10, "%d", tpTCPPort);
	...
	if (tcp_connect_resolve_hostname(server))
	{
		...
		g_server_address->sin_port = htons((uint16) tpTCPPort);
