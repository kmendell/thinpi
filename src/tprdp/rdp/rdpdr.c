...
rdpdr_send_client_name_request(void)
{
	...
	if (NULL == tpRDPDRClientName)
	{
		tpRDPDRClientName = tpHostname;
	}

	s_realloc(&name, 512 * 4);
	s_reset(&name);
	out_utf16s(&name, tpRDPDRClientName);
	s_mark_end(&name);
...
rdpdr_send_client_device_list_announce(void)
{
	...
	out_uint32_le(s, tpRDPDRNumDevices);

	for (i = 0; i < tpRDPDRNumDevices; i++)	/* DEVICE_ANNOUNCE */
	{
