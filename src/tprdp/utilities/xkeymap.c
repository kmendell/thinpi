...
xkeymap_from_locale(const char *locale)
{
	...
	{
		fclose(fp);
		STRNCPY(tpKeymap, str, sizeof(tpKeymap));
		xfree(str);
		return True;
	}
...
xkeymap_read(char *mapname)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL)
	{
		...
		{
			tpKBLayout = strtoul(line + sizeof("map ") - 1, NULL, 16);
			logger(Keyboard, Debug, "xkeymap_read(), Keylayout 0x%x", tpKBLayout);
			continue;
		}
		...
		{
			tpKBType = strtol(line + sizeof("keyboard_type ") - 1, NULL, 16);
...
xkeymap_init(void)
{
	...
	if (strcmp(tpKeymap, "none"))
	{
		if (xkeymap_read(tpKeymap))
			keymap_loaded = True;
	}
