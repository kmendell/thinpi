...
extern RD_BOOL tpUserQuit;
extern RD_BOOL g_exit_mainloop;

extern window_size_type_t tpWindowType;
...
extern RD_BOOL tpSeamlessRDP;
extern RD_BOOL tpSeamlessPersistent; 

extern uint32 tpEmbedWindow;
...
extern RD_BOOL tpSelfBackstore; 
...
{ \
	XFillRectangle(g_display, tpSelfBackstore ? g_backstore : g_wnd, g_gc, x, y, cx, cy); \
}
...
/* colour maps */
extern RD_BOOL tpColMap;
static Colormap g_xcolmap;
static uint32 *g_colmap = NULL;

#define TRANSLATE(col)		( tpServerDepth != 8 ? translate_colour(col) : tpColMap ? col : g_colmap[col] )
...
translate_image(int width, int height, uint8 * data)
{
	...
	/* todo */
	if (tpServerDepth == 32 && g_depth == 24)
	{
		return data;
	}
	...
	{
		if ((g_depth == 15 && tpServerDepth == 15) ||
		    (g_depth == 16 && tpServerDepth == 16) ||
		    (g_depth == 24 && tpServerDepth == 24))
			return data;
	}
...
select_visual(int screen_num)
{
	...
	if (tpServerDepth == -1)
	{
		tpServerDepth = DisplayPlanes(g_display, DefaultScreen(g_display));
	}
	...
	if (vmatches != NULL)
	{
		for (i = 0; i < visuals_count; ++i)
		{
					       ...
					       (visual_info->blue_mask == 0xff))))
			{
				...
				g_no_translate_image = (visual_info->depth == tpServerDepth);
				if (g_no_translate_image)
...
ui_init(void)
{
	...
	logger(GUI, Debug,
	       ...
	       tpServerDepth, g_depth, g_bpp, g_xserver_be, g_host_be);
	...
	if ((!tpSelfBackstore) && (DoesBackingStore(g_screen) != Always))
	{
		logger(GUI, Warning, "External BackingStore not available. Using internal");
		tpSelfBackstore = True;
	}
...
get_window_attribs(XSetWindowAttributes * attribs)
{
	...
	attribs->backing_store = tpSelfBackstore ? NotUseful : Always;
	if (g_has_wm) {
		attribs->override_redirect = 0;
	} else {
		attribs->override_redirect = tpFullscreen;
	}
...
get_input_mask(long *input_mask)
{
	*input_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask |
	...
	if (tpFullscreen || tpKeyboardGrab)
...
get_sizehints(XSizeHints * sizehints, uint32 width, uint32 height)
{
	...
	if (tpSeamlessRDP || !g_dynamic_session_resize)
	{
...
ui_create_window(uint32 width, uint32 height)
{
	...
	if (tpXPos < 0 || (tpXPos == 0 && (tpPos & 2)))
		tpXPos = WidthOfScreen(g_screen) + tpXPos - width;
	if (tpYPos < 0 || (tpYPos == 0 && (tpPos & 4)))
		tpYPos = HeightOfScreen(g_screen) + tpYPos - height;

	value_mask = get_window_attribs(&attribs);

	g_wnd = XCreateWindow(g_display, RootWindowOfScreen(g_screen), tpXPos, tpYPos, width,
	...
	if ((tpSelfBackstore) && (g_backstore == 0))
	{
	...
	XStoreName(g_display, g_wnd, tpTitle);
	ewmh_set_wm_name(g_wnd, tpTitle);
	...
	if (tpEmbedWindow)
	{
		XReparentWindow(g_display, g_wnd, (Window) tpEmbedWindow, 0, 0);
	}
...
xwin_toggle_fullscreen(void)
{
	...
	if (!tpFullscreen || (windowed_width == 0 || windowed_height == 0))
	{
	...
	/* If the window manager overrides our window size request, we trust
	   ...
	   ourselves setting tpPendingResize. */
	if (tpFullscreen)
	{
		tpPendingResize = True;
...
handle_button_event(XEvent xevent, RD_BOOL down)
{
	...
	if (xevent.xbutton.y < tpButtonSize)
	{
		/*  Check from right to left: */
		if (xevent.xbutton.x >= attr.width - tpButtonSize)
		{
			/* The close button, continue */
			;
		}
		else if (xevent.xbutton.x >= attr.width - tpButtonSize * 2)
		{
		...
		else if (xevent.xbutton.x >= attr.width - tpButtonSize * 3)
		{
		...
		else if (xevent.xbutton.x <= tpButtonSize)
		{
		...
		else
		{
			...
			if (xevent.type == ButtonPress)
			{
				if (!tpFullscreen && tpHideDecorations && !g_using_full_workarea)
				{
					g_moving_wnd = True;
	...
	while ((XPending(g_display) > 0) && events++ < 20)
	{
		...
		switch (xevent.type)
		{
			...
			case FocusIn:
				...
				g_focused = True;
				reset_modifier_keys();
				if (tpKeyboardGrab && g_mouse_in_wnd)
			...
			case ConfigureNotify:
				...
				if (xevent.xconfigure.window == DefaultRootWindow(g_display))
				{
					...
					if (tpWindowType == PercentageOfScreen
					    || tpWindowType == Fullscreen || tpFullscreen)
					{
...
#endif
				if (xevent.xconfigure.window == g_wnd && !tpSeamlessRDP
					    && is_g_wnd_mapped)
				{
...
process_pending_resize()
{
	...
	if (timeval_is_set(&tpPendingResizeDeferTimer) &&
	    time_difference_in_ms(tpPendingResizeDeferTimer, now) >= 2000)
	{
		tpPendingResizeDeferTimer.tv_sec = tpPendingResizeDeferTimer.tv_usec = 0;
		tpPendingResizeDefer = False;
	}

	if (tpPendingResizeDefer == True)
		return False;

	/* Set up width and height for new session */
	if (tpFullscreen || tpSeamlessRDP)
	{
		/* follow root window size */
		tpSessionWidth = WidthOfScreen(g_screen);
		tpSessionHeight = HeightOfScreen(g_screen);
		if (tpWindowType == PercentageOfScreen)
		{
	...
	{
		/* Follow window size */
		tpSessionWidth = g_window_width;
		tpSessionHeight = g_window_height;
	}
	...
	else
	{
		...
		rdpedisp_set_session_size(tpSessionWidth, tpSessionHeight);
...
ui_select(int rdp_socket)
{
	...
	while (g_exit_mainloop == False && rdp_socket_has_data == False)
	{
		...
		if (tpPendingResize == True && g_dynamic_session_resize)
		{
...
ui_create_bitmap(int width, int height, uint8 * data)
{
	...
	tdata = (tpColMap ? data : translate_image(width, height, data));
	  ...
	  /* brush */ BRUSH * brush, uint32 bgcolour, uint32 fgcolour)
{
	...
	if (tpSelfBackstore)
		XCopyArea(g_display, g_backstore, g_wnd, g_gc, x, y, cx, cy, x, y);
	ON_ALL_SEAMLESS_WINDOWS(XCopyArea,
				(g_display, tpSelfBackstore ? g_backstore : g_wnd, sw->wnd, g_gc,
	     ...
	     /* src */ int srcx, int srcy)
{
	...
	ON_ALL_SEAMLESS_WINDOWS(XCopyArea,
				(g_display, tpSelfBackstore ? g_backstore : g_wnd,
...
ui_seamless_begin(RD_BOOL hidden)
{
	...
	{
		seamless_send_spawn(tpSeamlessShellSpawn);
		tpSeamlessShellSpawn[0] = 0;
	}

	seamless_send_persistent(tpSeamlessPersistent);
}
...
ui_seamless_ack(unsigned int serial)
{
	...
	for (sw = g_seamless_windows; sw; sw = sw->next)
	{
		if (sw->outstandintpPosition && (sw->outpos_serial == serial))
		{
