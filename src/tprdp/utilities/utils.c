...
int utils_locale_to_utf8(const char *src, size_t is, char *dest, size_t os)
{
	...
	if (strncmp(tpCodepage, "UTF-8", strlen("UTF-8")) == 0)
	...
	{
		if ((iconv_h = iconv_open("UTF-8", tpCodepage)) == (iconv_t)-1)
		{
			logger(Core, Warning,
				   "utils_string_to_utf8(), iconv_open[%s -> %s] fail %p", tpCodepage,
				   "UTF-8", iconv_h);
								...
								RD_BOOL hostname_mismatch, const char *hostname)
{
	...
	rv = gnutls_verify_stored_pubkey(certcache_fn, NULL, hostname, "tprdp", type, &cert_list[0], 0);
	...
	if (rv == GNUTLS_E_CERTIFICATE_KEY_MISMATCH)
	{
		...
		//insert the new tofu agreement dialog prompt
	...
	/* show dialog */
	int *tofu = system("/thinpi/thinpi-tofu");
	if (tofu == 0) {
		response = "yes";
	} else {
		response = "no";
	...
	rv = gnutls_store_pubkey(certcache_fn, NULL, hostname, "tprdp", type, &cert_list[0], exp_time, 0);
	if (rv != GNUTLS_E_SUCCESS)
	{
